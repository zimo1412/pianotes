  ...
  return (
    <div style={{ overflowX: 'auto', margin: '0 -24px', padding: '0 24px' }}>
      <div style={{ display: 'flex', gap: 4, minWidth: 'fit-content', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginRight: 4, fontFamily: '"Noto Serif SC", serif', fontWeight: 800, fontSize: 10, color: styles.textFaint, letterSpacing: '0.06em' }}>
          {weekdays.map((d, i) => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, fontSize: 11, color: styles.textFaint, fontFamily: '"Noto Serif SC", serif', fontWeight: 700, flexWrap: 'wrap' }}>
...
function StatCard({ styles, label, icon, highlight, children }) {
  return (
    <div style={{ background: highlight ? styles.bgHighlight : styles.bgCard, border: `1px solid ${highlight ? styles.accent : styles.border}`, padding: 16 }}>
      <div style={{ fontSize: 10, letterSpacing: '0.08em', color: styles.textFaint, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4, fontFamily: '"Noto Serif SC", serif', fontWeight: 700 }}>
        {icon}
        {label}
      </div>
      <div>{children}</div>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.15em', color: styles.accent, marginBottom: 12, fontFamily: '"Noto Serif SC", serif', fontWeight: 800 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.15em', color: styles.accent, marginBottom: 8, fontFamily: '"Noto Serif SC", serif', fontWeight: 800 }}>
          {t('progress.repertoireTitle')}
        </div>
        <p style={{ fontSize: 12, color: styles.textFaint, lineHeight: 1.55, fontFamily: '"Noto Serif SC", serif', fontWeight: 600, margin: 0 }}>
          {t('progress.repertoireIntro')}
        </p>
      </div>

      {totalFilledTitles === 0 && (
        <div style={{ padding: '16px 12px', background: styles.bgCard, border: `1px solid ${styles.border}`, marginBottom: 24, fontSize: 13, color: styles.textMuted, fontFamily: '"Noto Serif SC", serif', fontWeight: 600 }}>
      ...
      {taggedGroups.map(({ tag, items }) => (
        <div key={tag} style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 11, letterSpacing: '0.15em', color: styles.accent, marginBottom: 10, fontFamily: '"Noto Serif SC", serif', fontWeight: 800 }}>
            {tag}
          </div>
          <div>
            {items.map((item) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <Check size={16} strokeWidth={3} style={{ color: styles.accent, flexShrink: 0, marginTop: 3 }} aria-hidden />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: styles.textFaint, marginTop: 4, lineHeight: 1.45, fontFamily: '"Noto Serif SC", serif', fontWeight: 600 }}>
                      {item.stageHint}
                    </div>
                  </div>
                </div>
      ...
      {allDone && (
        <div style={{ background: styles.bgRest, border: `1px solid ${styles.borderRest}`, padding: 20, marginTop: 24, textAlign: 'center' }}>
          <div style={{ fontSize: 16, color: styles.text, marginBottom: 8, fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic', fontWeight: 700 }}>
            {t('progress.curriculumDoneTitle')}
          </div>
          <p style={{ fontSize: 13, color: styles.textMuted, lineHeight: 1.6, fontWeight: 600 }}>
